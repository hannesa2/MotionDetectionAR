...
public class MotionDetectionActivity extends RajawaliActivity implements
        ...
        public void onManagerConnected(int status) {
            switch (status) {
                case LoaderCallbackInterface.SUCCESS: {
                    ...
                    csd = new com.timegalore.motiondetectionar.CAMShiftDetection(loadedImage, initialWindow, 10, 4,
                            10, 0.01);
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mRenderer = new com.timegalore.motiondetectionar.OpenGLRenderer(this);
    ...
    private void augmentImage(Mat mRgba, RotatedRect rr, int mSensorX, int mSensorY, int mSensorZ, int leftRightRot) {
        ...
        Log.d(TAG, "x:" + mSensorX + " y:" + mSensorY + " z:" + mSensorZ + " rot:" + df.format(lrTiltAngleInRadians) + " fb:" + df.format(fbTiltAngleInRadians) + " lr:" + df.format(leftRightRot));
        ...
        double cs = Math.sqrt(rr.boundingRect().area());

        mRenderer.setCubeSize(2 * cs / 480); // 0.6 for pegasus
    }

    public void printMatDetails(String name, Mat m) {
        Log.d(TAG, name + " - " + "c:" + m.channels() + ",cols:" + m.cols() + ",dep:" + m.depth() + ",rows:" + m.rows() + ",type:" + m.type() + ",w:" + m.width() + ",h:" + m.height());
    }
    ...
    public boolean onTouch(View v, MotionEvent event) {
        ...
        if (viewMode == VIEW_MODE_CAPTUREIMAGE) {
            ...
            Rect roi = new Rect(new Point(w * 1 / 3 + 1, h * 1 / 3 + 1), new Point(w * 2 / 3, h * 2 / 3));
            ...
            csd = new com.timegalore.motiondetectionar.CAMShiftDetection(loadedImage, initialWindow, 10, 4, 10, 0.01);
