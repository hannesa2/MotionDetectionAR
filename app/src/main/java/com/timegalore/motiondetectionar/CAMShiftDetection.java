...
public class CAMShiftDetection {
    ...
    private final Rect g_firstWindow;
    ...
    public CAMShiftDetection(Mat targetImage, Rect initialWindow,
        ...
        g_hist = getImageHistogram(getHue(targetImage), targetImage.size(), 10, 0, 180);
    ...
    public RotatedRect CAMShift(Mat in) {
        ...
        Mat clarifiedBackProjection = clarifyDetectedAreas(backProjection, g_erosion_kernel_size, g_erosion_level);

        validateWindow();

        RotatedRect rr = doCamShift(clarifiedBackProjection, g_initialWindow, g_termcrit_count, g_termcrit_eps);
    ...
    private Mat getImageHistogram(Mat huesImage, Size size, int buckets, float minRange, float maxRange) {
    ...
    private Mat clarifyDetectedAreas(Mat in, int erosion_kernel_size, int erosion_level) {
        Mat out = new Mat(in.size(), in.type());

        Mat eroded_kernel = Imgproc.getStructuringElement(Imgproc.MORPH_RECT,
                new Size(erosion_kernel_size, erosion_kernel_size), new Point(erosion_kernel_size / 2, erosion_kernel_size / 2));

        Imgproc.erode(in, out, eroded_kernel, new Point(-1, -1), erosion_level, Core.BORDER_DEFAULT, new Scalar(0));

        return out;
    }

    private RotatedRect doCamShift(Mat in, Rect initialWindow, int termcrit_count, double termcrit_eps) {
        TermCriteria termcrit = new TermCriteria(TermCriteria.MAX_ITER | TermCriteria.EPS, termcrit_count, termcrit_eps);
    ...
    private Mat getBackProjection(Mat in, Mat histogram, int minRange, int maxRange, double scale) {
        ArrayList<Mat> images = new ArrayList<Mat>();
        images.add(in);

        Mat backproject = new Mat(in.size(), CvType.CV_8UC1);

        Imgproc.calcBackProject(images, new MatOfInt(0), histogram, backproject, new MatOfFloat(minRange, maxRange), scale);
